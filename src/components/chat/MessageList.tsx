...
function getToolLabel(toolName: string, args: any, done: boolean): string {
  ...
  if (toolName === "str_replace_editor") {
    if (command === "create") return done ? `Created ${path}` : `Creating ${path}…`;
    if (command === "str_replace" || command === "insert") return done ? `Updated ${path}` : `Updating ${path}…`;
    if (command === "view") return `Reading ${path}`;
  }

  if (toolName === "file_manager") {
    if (command === "rename") return done ? `Renamed ${path} → ${newPath}` : `Renaming ${path}…`;
    if (command === "delete") return done ? `Deleted ${path}` : `Deleting ${path}…`;
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const label = getToolLabel(tool.toolName, (tool as any).args, !!done);
                            return (
